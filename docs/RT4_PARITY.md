...
| `rt add-images` (`--web-search N` per unità, `--units`) | `POST /lessons/{id}/images` (`web_search`, `units`) → job `add_images`; `GET /lessons/{id}/images`, `/assets/images/{nome}` | Immagini della lezione (`/lezioni/{id}/immagini`) | ✅ `test_row_add_images`, `test_row_add_images_web_search_per_unit` | ✅ `test_images_job_persists` | ✅ `recall-images-bot.spec.ts` (PDF, job, anteprima; immagini per unità sulle unità scelte) |
...
| `rt recall --channel telegram`, `/quit` nel bot | `GET /recall/session`, `POST /recall/session/end`, `GET /recall/telegram`, `POST /recall/telegram/start`, `/recall/telegram/sessions/{id}/stop` | Termina sessione, luogo Telegram/Qui, Interrompi (`/lezioni/{id}/recall`) | — (avvio e interruzione passano dal daemon) | ✅ `test_api_recall_sessions.py` (registro nel DB, Bot API finta) | ✅ `recall-sessions.spec.ts` (riepilogo, slitta da tastiera, bot finto) |
